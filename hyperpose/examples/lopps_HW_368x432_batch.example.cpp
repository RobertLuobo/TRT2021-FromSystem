...
int main(int argc, char** argv)
{
    ...
    auto beg = clk_t::now();
    {
        ...
        std::cout << batch.size() << " images got processed. FPS = "
       	...
       	//std::cout << "lopps-resnet50-V2-HW=368*432.onnx fp32 " << "batchsize:" << batch.size() << "\n";
       	std::cout << "openpose-coco-V2-HW=368x656.onnx  fp32 " << "batchsize:" << batch.size() << "\n";
        for (size_t i = 0; i < batch.size(); ++i) {
            ...
            cv::imwrite("output_fp32_paf_coco_" + std::to_string(i) + ".png", batch[i]);
        }
    }
}
