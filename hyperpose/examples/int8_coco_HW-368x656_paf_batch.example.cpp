...
// Model flags
DEFINE_string(model_file,"/hyperpose/conver_to_trt/models_save/hyperpose_coco_368x656_int8.trt" , "Path to uff model.");
...
int main(int argc, char** argv)
{
    ...
    auto beg = clk_t::now();
    {
        ...
        std::cout << batch.size() << " images got processed. FPS = "
       	...
       	std::cout << "openpose-coco-V2-HW=368x656.onnx hyperpose_coco_368x656_int8.trt  int8 " << "batchsize:" << batch.size() << "\n";
        for (size_t i = 0; i < batch.size(); ++i) {
            ...
            cv::imwrite("coco_paf_output_int8__" + std::to_string(i) + ".png", batch[i]);
        }
    }
}
